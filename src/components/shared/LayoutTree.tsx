...
export function LayoutTree({ data }: FileTreeProps) {
    ...
    function flattenTree(nodes: TreeNode[], level = 1): FlatNode[] {
        let result: FlatNode[] = [];
    ...
    function handleSelect(id: string, e: React.MouseEvent) {
        ...
        if (e.shiftKey && lastSelectedId) {
            const start = flatTree.findIndex(n => n.id === lastSelectedId);
            const end = flatTree.findIndex(n => n.id === id);

            if (start === -1 || end === -1) return;

            const [from, to] = start < end ? [start, end] : [end, start];
            const range = flatTree.slice(from, to + 1).map(n => n.id);
